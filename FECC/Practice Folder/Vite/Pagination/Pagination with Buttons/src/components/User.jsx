...
const itemPerPage=50;
    ...
    useEffect(()=>{
        const FetchUserDetails= async()=>{
            ...
            if(resp.status!== 200){throw new Error("API Error: " );}
    ...
    return(
        ...
        <PaginationController data={totalData} itemPerPage={itemPerPage}/>
        </div>
            ...
            {totalData.map((item,index)=>{
                ...
                return (
                    <div className="UserCardBox" key={index}>
                        ...
                        <p>{email}</p>
                        <p className="commentOverflowControl"><strong>Comment: </strong>{body}</p>
                    </div>
                )
            })}
        </div>
